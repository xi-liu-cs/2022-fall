...
<small>
    ...
    <br> <input type=range id=ci value="100",min="0",max="100"> Cloud Intensity
</small>
`;
...
S.setFragmentShader(\`
   ...
   // COMPUTE AND SHADE THE NOISE-BASED CLOUDS

   vec3 shadeCloud(vec3 V, vec3 W, float t) {
      if (V.y + t * W.y > 10.) {
         t = (10. - V.y) / W.y;
         vec3 cloudPoint = V + t * W;
         float layer = abs(noise(cloudPoint * .01 - vec3(uTime * .1)));
         layer *= abs(noise(cloudPoint * .02 + vec3(uTime * .1)));
   ...
   vec3 shadeSurface(vec3 W, vec3 surfacePoint) {
      ...
      rc += shadeCloud(surfacePoint, R, rtMin) * uLc[0];
