...
void same_cpu_remain_handler(queue * q)
{
  ...
  int min_i = q->front;
  for(int i = 1; i < q->cur && prev->cpu_remain == cur->cpu_remain; ++i)
  {
    prev = q->a[(q->front + i) % q->cap];
    cur = q->a[(q->front + i + 1) % q->cap];
    if(cur->pid < q->a[min_i]->pid)
      min_i = (q->front + i + 1) % q->cap;
  }
...
void shortest_remaining_job_first(file * fp_out, proc ** proc_array, int n_proc)
{
  ...
  while(finish < n_proc)
  {
    ...
    if(finish == n_proc)
      break;
    queue * ready_and_run = queue_malloc(n_proc);
    memcpy(ready_and_run->a, q->a, q->cur * sizeof(proc *));
    ready_and_run->cur = q->cur + 1;
    push(ready_and_run, cur_run);
    qsort(ready_and_run->a, ready_and_run->cur, sizeof(proc *), compare_cpu_remain);
    same_cpu_remain_handler(ready_and_run);
    if(!is_empty(ready_and_run))
    {
      if(cur_run && front(q) != cur_run)
      {
        cur_run->state = ready;
        push(q, cur_run);
      }
