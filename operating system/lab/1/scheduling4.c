...
void first_come_first_served(file * fp_out, proc ** proc_array, int n_proc)
{
  ...
  while(finish < n_proc)
  {
    ...
    if(finish == n_proc)
      break;
    if(!queue_link_is_empty(q_cur_cycle) && !cur_run)
    {
      queue_link * q_cur_cycle2 = sort_queue_link(q_cur_cycle, compare_pid);
      free(q_cur_cycle);
      q_cur_cycle = q_cur_cycle2;
      cur_run = q_cur_cycle->front->p;
      cur_run->state = running;
      queue_link_pop(q_cur_cycle);
      queue_link_erase(q, cur_run);
    }
    else if(queue_link_is_empty(q_cur_cycle) && !queue_link_is_empty(q) && !cur_run)
    {
      cur_run = q->front->p;
      cur_run->state = running;
      queue_link_pop(q);
...
{
  queue_link * q = queue_link_malloc(n_proc); /* queue of ready processes */
  int finish = 0, /* count of currently finished processes */
  cycle = 0,
  cpu_work = 0,
  ...
  while(finish < n_proc)
  {
    ...
    if(finish == n_proc)
      break;
    printf("\ncycle = %d\n", cycle);
    print_queue_link(q_cur_cycle);
    printf("\nq\n");
    print_queue_link(q);
    printf("\n");
    if(queue_link_is_size_greater_than_1(q_cur_cycle)
    && !queue_link_is_empty(q_cur_cycle) && !cur_run)
    {
      printf("1st if in cycle = %d\n", cycle);
      queue_link * q_cur_cycle2 = sort_queue_link(q_cur_cycle, compare_pid);
      free(q_cur_cycle);
      q_cur_cycle = q_cur_cycle2;
      cur_run = q_cur_cycle->front->p;
    ...
    else if(!queue_link_is_empty(q) && !cur_run)
    {
      printf("2nd if in cycle = %d\n", cycle);
      cur_run = q->front->p;
      cur_run->state = running;
      queue_link_pop(q);
    }
